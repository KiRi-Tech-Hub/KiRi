...
const projects = [
    {
        ...
        tags: ['React 19 + Vite + TypeScript + Tailwind CSS', 'Node.js + TypeScript + Redis', 'Monaco Editor'],
    ...
    },
    {
        category: 'AI / ML',
        title: 'SafeQuest',
        client: 'EdTech · India · 2025',
        description: 'AI-powered educational storytelling platform that teaches children life values — empathy, courage, and kindness — through interactive, personalized, choice-driven adventures.',
        tags: ['React', 'Node.js', 'MongoDB', 'Groq API'],
        result: 'Real-time narration & parental controls',
        hasShowcase: true,
...
/* ── SafeQuest slides data ─────────────────────────────────────────────── */
const safequestSlides = [
    {
        src: '/our project/safequest1.png',
        title: 'Welcome Screen',
        description: 'SafeQuest greets young adventurers with a vibrant, welcoming interface that invites them to embark on a personalized story journey built around real-world values.',
    },
    {
        src: '/our project/safequest2.png',
        title: 'Story World',
        description: 'Richly illustrated story worlds powered by AI — each environment reflects the moral theme of the current adventure, keeping children fully immersed.',
    },
    {
        src: '/our project/safequest3.png',
        title: 'Character Creation',
        description: 'Kids design their own hero with dynamic character expressions and personality traits that evolve as they make choices throughout the story.',
    },
    {
        src: '/our project/safequest4.png',
        title: 'Choice-Based Gameplay',
        description: 'Every decision shapes the outcome — branching narratives generated by Groq API ensure no two playthroughs are the same, reinforcing critical thinking.',
    },
    {
        src: '/our project/safequest5.jpeg',
        title: 'Real-Time Narration',
        description: 'Coqui TTS brings stories to life with expressive, real-time voice narration that adapts its tone to match the story\'s emotional beats.',
    },
    {
        src: '/our project/safequest6.png',
        title: 'Learning Values Dashboard',
        description: 'A child-friendly progress tracker celebrates the life values explored — courage, empathy, kindness — with badges and milestone animations.',
    },
    {
        src: '/our project/safequest7.png',
        title: 'Parental Controls',
        description: 'Parents get a dedicated dashboard to monitor learning progress, session history, and story outcomes — ensuring safe, meaningful screen time.',
    },
];

/* ── Accent colour tokens (static so Tailwind JIT never purges them) ── */
const accentTokens: Record<string, { bg20: string; text: string; hex: string }> = {
    amber: { bg20: 'rgba(245,158,11,0.15)', text: '#f59e0b', hex: '#f59e0b' },
    emerald: { bg20: 'rgba(16,185,129,0.15)', text: '#34d399', hex: '#10b981' },
};

/* ── Slideshow Modal ───────────────────────────────────────────────────── */
type ShowcaseProps = {
    onClose: () => void;
    slides: typeof codehiveSlides;
    brand: string;
    accent: 'amber' | 'emerald';
};

function ProjectShowcase({ onClose, slides, brand, accent }: ShowcaseProps) {
        ...
        >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.07] flex-shrink-0">
                <div className="flex items-center gap-3">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: accentTokens[accent].bg20 }}>
                        <MonitorPlay size={14} style={{ color: accentTokens[accent].text }} />
                    </div>
                    <div>
                        <p className="text-[9px] font-black uppercase tracking-[0.25em] text-white/30">{brand} · Project Showcase</p>
                    </div>
                </div>
            ...
            <div className="flex-shrink-0 border-t border-white/[0.07] px-6 py-5">
                <div className="max-w-2xl mx-auto text-center">
                    <p className="text-white/50 text-sm leading-relaxed mb-1">
                        <span className="font-semibold" style={{ color: accentTokens[accent].text }}>{slide.title}</span>
                    </p>
                    <p className="text-white/35 text-xs leading-relaxed mb-4">{slide.description}</p>
...
function CodeHiveShowcase({ onClose }: { onClose: () => void }) {
    return <ProjectShowcase onClose={onClose} slides={codehiveSlides} brand="CodeHive" accent="amber" />;
}
function SafeQuestShowcase({ onClose }: { onClose: () => void }) {
    return <ProjectShowcase onClose={onClose} slides={safequestSlides} brand="SafeQuest" accent="emerald" />;
}
    ...
    return (
        ...
        >
            ...
            >
                ...
                <div className={`h-[2px] w-full ${!project.hasShowcase ? 'bg-slate-900 dark:bg-white' :
                    project.accentColor === 'emerald' ? 'bg-emerald-500' : 'bg-amber-500'
                    }`} />
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-5 flex-wrap">
                                ...
                                {project.hasShowcase && onShowcase && (
                                    <button
                                        onClick={onShowcase}
                                        className={`group inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold transition-all duration-200 ${project.accentColor === 'emerald'
                                            ? 'border border-emerald-500/40 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500 hover:border-emerald-500 hover:text-white'
                                            : 'border border-amber-500/40 text-amber-600 dark:text-amber-400 hover:bg-amber-500 hover:border-amber-500 hover:text-white'
                                            }`}
                                    >
...
export default function Portfolio() {
    ...
    const [safequestOpen, setSafequestOpen] = useState(false);
    ...
    return (
        <div className="min-h-screen bg-[#f8f8fb] dark:bg-[#0a0a0b] pt-20">
            ...
            {/* ── SafeQuest Showcase Modal ─────────────────── */}
            <AnimatePresence>
                {safequestOpen && (
                    <SafeQuestShowcase onClose={() => setSafequestOpen(false)} />
                )}
            </AnimatePresence>
        </div>
    );
